...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { lead, match, recipient: clientRecipient, test } = await req.json()
    ...
    if (test) {
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Labplus Leads <onboarding@resend.dev>',
          to: [finalRecipient],
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
          ...
          <tr><td style="padding: 8px; border-bottom: 1px solid #ddd;"><strong>Volume:</strong></td><td style="padding: 8px; border-bottom: 1px solid #ddd;">${lead.volume}</td></tr>
        </table>
        ...
        <div style="margin-top: 24px; padding: 16px; background-color: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
          ...
          <p style="margin: 4px 0;"><strong>Equipamento Recomendado:</strong> ${match.equipment_name}</p>
          <p style="margin: 4px 0;"><strong>Fabricante:</strong> ${match.provider}</p>
          <p style="margin: 4px 0;"><strong>Condições:</strong> ${match.payment_conditions}</p>
        </div>
        `
            : ''
        }
